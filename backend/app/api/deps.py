...
def get_current_active_staff(
    ...
    if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR, UserRole.VENDEDOR]:
        raise HTTPException(
            status_code=403, detail="Solo el personal autorizado puede descargar el lector NFC"
        )
    return current_user
