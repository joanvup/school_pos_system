  ...
  <div class="flex flex-col h-full gap-6 lg:flex-row p-2">
    ...
    <div class="flex-1 space-y-4">
      <div class="flex justify-between items-center">
        ...
        <input v-model="search" type="text" placeholder="🔍 Buscar producto..." class="w-64 p-2 border rounded-xl shadow-sm outline-none focus:ring-2 focus:ring-primary">
      </div>
      
      <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 overflow-y-auto max-h-[65vh] pb-10">
        <div 
          v-for="product in filteredProducts" :key="product.id" 
          @click="addToCart(product)"
          class="bg-white p-4 rounded-2xl shadow-sm border border-transparent hover:border-primary hover:shadow-md cursor-pointer transition-all group"
        >
          <div class="h-20 w-20 rounded-2xl mx-auto mb-3 flex items-center justify-center text-4xl group-hover:scale-110 transition overflow-hidden">
    <!-- Si tiene imagen real, la mostramos -->
    <img 
        v-if="product.image_url" 
        :src="'http://127.0.0.1:8000' + product.image_url" 
        class="h-full w-full object-cover"
    >
    <!-- Si no tiene, usamos el icono por categoría (fallback) -->
    <span v-else class="bg-gray-50 h-full w-full flex items-center justify-center">
        {{ getCategoryIcon(product.category_rel?.name) }}
    </span>
</div>
          <h3 class="font-bold text-gray-700 text-sm text-center leading-tight h-10">{{ product.name }}</h3>
          <div class="mt-2 text-center">
            <span class="text-primary font-black text-lg">{{ formatMoney(product.price) }}</span>
            <p class="text-[10px] text-gray-400 font-bold uppercase mt-1">Stock: {{ product.stock }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border p-4">
        <h3 class="text-xs font-black text-gray-400 uppercase mb-3">Últimas 5 Ventas</h3>
        <div class="space-y-2">
            <div v-for="sale in recentSales" :key="sale.id" class="flex justify-between items-center text-sm border-b pb-2 last:border-0">
                <div>
                    <span class="font-bold text-gray-700">{{ sale.comprador }}</span>
                    <span class="text-[10px] text-gray-400 ml-2">{{ new Date(sale.timestamp).toLocaleTimeString() }}</span>
                </div>
                <div class="flex items-center gap-3">
                    <span :class="sale.status === 'reversed' ? 'line-through text-red-400' : 'text-green-600 font-black'">
                        {{ formatMoney(sale.amount) }}
                    </span>
                    <button v-if="authStore.isAdmin && sale.status !== 'reversed'" @click="handleReverse(sale.id)" class="text-[10px] bg-red-50 text-red-600 px-2 py-1 rounded font-bold">
                        REVERSAR
                    </button>
                </div>
            </div>
